...
class MarketUser(models.Model):
    ...
    entity = models.CharField(max_length=50, blank=True, null=True) # company or institution
...
class Region(models.Model):
    ...
    iso3166 = models.CharField(max_length=5, validators=[MinLengthValidator(5)], null=True, blank=False)
...
class Landuse(models.Model):
    ...
    key = models.CharField(max_length=8, choices=Landuse.choices, unique_for_date='valid_from')
...
class AreaOffer(models.Model):
    ...
    status = models.TextChoices(
        choices=OfferStatus,
        default=OfferStatus.IN_PREPERATION
    )
    ...
    utilization = models.TextField(
        choices=AreaUtilization,
        default=AreaUtilization.NO_RESTRICTION
    )

    excluded_landuse = models.ManyToManyField(
        Landuse, related_name='offers'
    )
    ...
    preferred_regionality = models.TextField(
        choices=DeveloperRegionality,
        default=DeveloperRegionality.NO_RESTRICTION
    )
    ...
    shareholder_model = models.TextField(
        choices=ShareholderModel,
        default=ShareholderModel.NO_RESTRICTION
    )

    important_remarks = models.TextField()

    document_land_register = models.FileField(help_text=_('Auszug Grundbuch'))
    document_assingement_landowner = models.FileField(help_text=_('Beauftragung des Eigentümers'))
    document_declaration_of_consent_network_request = models.FileField(help_text=_('Einverständniserklärung der Netzanfrage'))
...
class AreaOfferConfirmation(models.Model):
    ...
    sale_amount = models.DecimalField(decimal_places=2, help_text=_('Angebotener Kaufpreis'))

    # if utilitization = LEASE
    lease_amount_single_payment = models.DecimalField(decimal_places=2, help_text=_('Einmalzahlung an Eigentümer bei Vertragsabschluss'))
    lease_amount_yearly_lease_year_one = models.DecimalField(decimal_places=2, help_text=_('Jährliche Pacht in Jahr 1'))
    contracted_term_month = models.PositiveSmallIntegerField(null=True, help_text=_('Vertragslaufzeit'))
    staggered_lease = models.TextField(choices=Ternary.choices, null=True, help_text=_('Staffelpacht möglich?'))
    share_of_income = models.TextField(choices=Ternary.choices, help_text=_('Bieten Sie Beteiligungen an laufenden Erlösen?'))
    shares_project_company = models.TextField(choices=Ternary.choices, help_text=_('Im Falle der Gründung einer Projektgesellschaft: Bieten Sie Beteiligungen an der Projektgesellschaft an?'))

    message_to_landowner = models.TextField(max_length=500, help_text=_('Ihre Nachricht an den Eigentümer zu dem Gebot'))
    message_to_platform = models.TextField(max_length=500, help_text=_('Ihre Nachricht an Agrario Energy'))
...
class AreaOfferAdministration(models.Model):
    ...
    # if landuse ECO
    lease_amount_single_payment = models.DecimalField(decimal_places=2) # currency of the offer
...
class FaqItem(models.Model):
    ...
    category = models.CharField(choices=FaqCategory.choices, default=FaqCategory.GENERAL)
